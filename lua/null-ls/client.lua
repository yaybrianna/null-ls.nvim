...
local should_attach = function(bufnr)
  ...
  if config.get().should_attach and not config.get().should_attach(bufnr) then
    return false
  end
...
local on_init = function(new_client, initialize_result)
  ...
  if config.get().on_init then
    config.get().on_init(new_client, initialize_result)
  end
  client = new_client
end
...
M.start_client = function(fname)
  local config = {
    name = "null-ls",
    root_dir = config.get().root_dir(fname) or vim.loop.cwd(),
    on_init = on_init,
    on_exit = on_exit,
...
M.setup_buffer = function(bufnr)
  ...
  local on_attach = config.get().on_attach
  if on_attach then
...
M.on_source_change = vim.schedule_wrap(function()
  ...
  utils.buf.for_each_bufnr(function(bufnr)
    if bufnr == current_bufnr then
      ...
      if state.has_conditional_sources() and bufname ~= "" and buftype == "" then
        state.register_conditional_sources()
      end
    else
      api.nvim_create_autocmd("BufEnter", {
        buffer = bufnr,
  ...
  if state.has_conditional_sources() then
